...
>(({ className, side = "top", align = "center", sideOffset = 6, ...props }, ref) => (
  // Portal, not an inline render - without it, a tooltip opened on a
  // trigger that lives inside any ancestor with overflow-x/y set
  // (the ranking table's own horizontal-scroll wrapper, notably) gets
  // silently clipped the instant it would extend past that ancestor's
  // bounds, cutting the explanation off mid-sentence rather than
  // actually floating above the page the way its shadow/z-index imply.
  // Radix's own collision detection already keeps it on-screen; it just
  // can't do anything about a clipping ancestor without being portaled
  // out of that ancestor's subtree first.
